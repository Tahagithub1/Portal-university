    ...
    <div class="row justify-content-center align-items-center h-100 p-5">
        <form action="{{ route('users.store') }}" method="POST" class="col-lg-5 card p-5 shadow-sm">
            ...
            <div class="mb-3">
                ...
                <input name="name" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="نام و نام خانوادگی را وارد کنید" value="{{ old('name') }}" /@required('name')>
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input name="username" class="form-control @error('username') is-invalid @enderror" id="username" placeholder="نام کاربری را وارد کنید" value="{{ old('username') }}"  /@required('username')>
                @error('username')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
           ...
           <input type="hidden" name="password" value="{{\Illuminate\Support\Facades\Hash::make("12345678")}}">
            <div class="mb-3">
                ...
                <select class="form-select" aria-label="Default select example" name="role" /@required('select')>
                    <option selected value="1">مسئول آموزش کل</option>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" aria-label="Default select example" name="collages_id"  /@required('select')>
